...
namespace logger {
	...
	inline void info(fmt::format_string<Args...> _format, Args &&..._args) {
		spdlog::get("logger_core")->info(_format, std::forward<Args>(_args)...);
	}
	template<typename... Args>
	inline void warn(fmt::format_string<Args...> _format, Args &&..._args) {
		spdlog::get("logger_core")->warn(_format, std::forward<Args>(_args)...);
	}
	template<typename... Args>
	inline void error(fmt::format_string<Args...> _format, Args &&..._args) {
		spdlog::get("logger_core")->error(_format, std::forward<Args>(_args)...);
	}
	template<typename... Args>
	inline void critical(fmt::format_string<Args...> _format, Args &&..._args) {
